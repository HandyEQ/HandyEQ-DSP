...
Fc = 20;
Fstop = 100; %kHz
Fp =(Fb)/Fs; %Passband freq (x*pi) (Normalized)
Fst = (Fstop)/Fs; %Stopband freq (y*pi) (Normalized)
Ap = 0.5; %Passband ripple
Ast = 90; %Stopband attenuation
N = 2; % Order of the filter
FilterInSeries = 1;

% --------- Working design -------- %
f = fdesign.lowpass('N,Fp,Ap',N,Fp,Ap); %'N,Fp,Ap' Working
Hdf1sos = design(f, 'cheby1', 'FilterStructure', 'df1sos'); % Working

% ---------------------------------%

% ---------- Beta designs --------- %

  %f = fdesign.lowpass('N,Fp,Fst,Ap',N,Fp,Fst,Ap);
  %Hdf1sos = design(f,'ellip', 'FilterStructure', 'df1sos'); 

  %f = fdesign.lowpass('N,Fp,Ap,Ast',N,Fp,Ap,Ast); % Works okay
  %Hdf1sos = design(f,'ellip', 'FilterStructure', 'df1sos'); %Works okay
  
  %f = fdesign.lowpass('N,Fst,Ast',N,Fst,Ast);
  %Hdf1sos = design(f,'cheby2', 'FilterStructure', 'df1sos'); 
...
Hdf1sos.CoeffAutoScale = false;
%Hdf1sos.OutputFracLength=17; %samma
Hdf1sos.CoeffWordLength = 32;
Hdf1sos.ScaleValueFracLength = 30;
...
Hdf1sosf.OutputFracLength=17; %Leker, ta bort om fel
...
Signed = 1; % Indicates from which range the data is represented, +- Range
Range = 1
...
for s = 1:3;
    ...
    a = fi([Hdf1sosf.sosMatrix(Counter)],Signed,Hdf1sosf.CoeffWordLength,Hdf1sosf.ScaleValueFracLength); % Converts to desired format, 16bit Word, 15 bit coefs, 1 bit rep of signed.
    z = a.bin;
   
    fprintf(fileID,'constant f%d_b%d : std_logic_vector(%d downto 0) := B"%s"; --b%d = %d\n', FilterInSeries,(Counter-1),(Hdf1sosf.CoeffWordLength-1),num2str(z),(Counter-1),Hdf1sosf.sosMatrix(Counter));
...
for s = 2:3; %Skips a0, because this is always 1.
     ...
     a = fi([Hdf1sosf.sosMatrix(Counter+3)],Signed,Hdf1sosf.CoeffWordLength,Hdf1sosf.ScaleValueFracLength);  % Converts to desired format, 16bit Word, 15 bit coefs, 1 bit rep of signed.
    z = a.bin;
   
    X = fprintf(fileID,'constant f%d_a%d : std_logic_vector(%d downto 0) := B"%s"; --a%d = %d\n', FilterInSeries,(Counter-1),(Hdf1sosf.CoeffWordLength-1),num2str(z),(Counter-1),Hdf1sosf.sosMatrix(Counter+3));
...
for s = 1:2; % Reads the scaling values and converts to binaries, ScaleValues_0 = Input, ScaleValues_1 = Output
    ...
    a = fi([Hdf1sosf.ScaleValues(Counter)],Signed,Hdf1sosf.InputWordLength,Hdf1sosf.InputFracLength);  % Converts to desired format, 16bit Word, 15 bit coefs, 1 bit rep of signed.
    z = a.bin;
   
    X = fprintf(fileID,'constant ScaleValue_f%d_%d : std_logic_vector(%d downto 0) := B"%s"; --ScaleValue_%d = %d\n',FilterInSeries,(Counter-1),(Hdf1sosf.InputWordLength-1),num2str(z),(Counter-1),Hdf1sosf.ScaleValues(Counter));
